...
export async function estimate(
  {amountString, inputCurrency, outputCurrency}: {amountString: string, inputCurrency: string, outputCurrency: string},
  {ethereumContext}: {ethereumContext: EthereumContextProps}): Promise<Arg> {
  const {publicAddress} = ethereumContext;

  if (!(inputCurrency === 'UST' && outputCurrency === 'USDC')) {
    throw new Error(`Unsupported currencies, only UST to USDC is supported. You requested ${inputCurrency} -> ${outputCurrency}`);
  }
